...
function reducer(state: DiaryItem[], action: Action) {
  ...
  switch (action.type) {
    ...
    case "EDIT": {
      newState = state.map((diaryItem) =>
        diaryItem.idx === action.diaryItems.idx
          ? {
              ...action.diaryItems,
            }
          : diaryItem
...
const DiaryIdxContext = createContext<MutableRefObject<number> | undefined>(
  undefined
);
const DiaryValueContext = createContext<DiaryItem[] | undefined>(undefined);
const DiaryDispatchContext = createContext<DiaryDispatch | undefined>(
  undefined
);

function DiaryProvider({ children }: { children: React.ReactNode }) {
  const isEmpty: boolean =
    JSON.parse(`${localStorage.getItem("diary")}`) == null ||
    JSON.parse(`${localStorage.getItem("diary")}`).length === 0;

  const initItems = isEmpty
    ? []
    : JSON.parse(`${localStorage.getItem("diary")}`);
  //고유 idx - 서비스 접속 시 저장된 데이터에서 불러오기 (sort에 사용 - 최신순, 오래된순)
  const initIdx = isEmpty
    ? 1
    : Math.max(JSON.parse(`${localStorage.getItem("diary")}`)[0].idx) + 1;
